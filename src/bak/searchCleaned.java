...
class Sudoku
{
	...
	//List of solved Boxes - only used once and not updated
	List<Box> solved = new ArrayList<>();
	//List of all units(ie groups) on the grid
	List<List<Box>> unitList = new ArrayList<>();
	
	//Stacks to keep track of changes - for backtracking
	//We push each change, so we can pop each to undo the last change
    Stack<Box> boxStack = new Stack();
    Stack<List<Integer>> valuesStack = new Stack(); 
    Stack<List<Box>> unsolvedStack = new Stack();
    Box breaker = new Box(-1); //used to bookmark sections to revert back to
    Box solvedBreaker = new Box(-1); //used to keep track of when unsolved list was changed
    ...
    class Box {
    	...
    	//Constructor
    	Box(int value) {
    		//Set value if known, otherwise set values to all possible options
    		...
    		if (value>=0) { //quick hack - lets me init breaker box earlier with Sudoku class
    			//Each box has (N-1) peers from each column and row (hence the times 2)
    			//plus (N-1) squared unique peers in each square
    			peers = new Box[(N-1)*2+(SIZE-1)*(SIZE-1)];
    		}
    	}
    	
    	void setPeers(int y, int x) {
    		//set each peer assuming myGrid is filled with Boxes 
    		int counter=0;
    		for (int i=0; i<N; i++) {
    			//Add row peers
    			if (i != x) {
    	...
    	boolean eliminate(int d) {
    		//eliminate a possible value and propagate constraints
    		//remember changes along the way
    		//returns false if it was an illegal elimination
    	...
    	boolean eliminateFromPeers(int d) {
    		//eliminate a value from every peer
    		//returns false is any move is illegal
    	...
    	boolean assign(int value) {
    		...
    		//Don't really need to keep track of solved Boxes
    		//They are only used once at the start + they can be using the unsolved Boxes
    		return true;
    	}
	...
	void convertMyGrid() {
		//write myGrid as original grid of integers
		for (int y=0; y<N; y++){
			for (int x=0; x<N; x++) {
	...
	void updateUnits() {
    	...
    	//but its easier to grasp while separated
    ...
    boolean CheckUnits() {
    	...
    	//returns false if only possible position is illegal move
		for (int i=0; i<unitList.size(); i++) {
			List<Box> unit = unitList.get(i);
			for (int n=1; n<=N; n++) { //Go through each possible number
				...
				if (places.size()==1) { //only one spot
					if (!places.get(0).assign(n)) { //assign and make sure its valid move
						return false;
					}
				}
			}
	...
	boolean revert() {
		//revert each change until hitting a breaker Box in the stack of changes
		//used in conjunction with search();
	...
	boolean search() {
    	//Depth first search for possible solution
		//Looks for Box with fewest values
		//Checks each value if it works - recursively searches to find out
		//Returns true once solution is found
		//backtracks each unsuccessful search
    ...
    {    	
    	...
    	//Reduce each Box as much as possible
    	...
    	//Some debugging printlns
        //print();   
        //System.out.println();
        //System.out.println("Solved: " + (N*N-unsolved.size())); //for debugging
        //System.out.println("Unsolved: " + unsolved.size()); //for debugging
    }
